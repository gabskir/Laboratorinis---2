...
bool lyginam_pazymius (studentas a, studentas b){
...
void skaitymas(vector<studentas>& grupe, studentas& temp){
    ...
    int strategija;
    cout << "\nPasirinkite strategija, kuria norite skaidyti i atskirtus vektorius: " << endl << "0 - dvieju nauju konteineriu " << endl << "1 - vieno naujo konteinerio" << endl;
    ...
    //pirma strategija kai naudojami du nauji vektoriai
    if (strategija == 0) {        
    ...
    //antra strategija - naudojant viena nauja vektoriu - pasidaveliai ir trinant is grupes
        vector<studentas> pasidaveliai;
...
void budas1 (vector<studentas>& grupe, vector<studentas>& broliaiSaunuoliai, vector<studentas>& pasidaveliai) {
    ...
    cout << "STUDENTU DALINIMAS I DVI GRUPES (DU KONTEINERIAI): " << dalinimas.count() << " s" << endl;
}
...
//     sort (grupe.begin(), grupe.end(), lyginam_pazymius);
//     auto start = std::chrono::high_resolution_clock::now();
//     std::stable_partition (grupe.begin(), grupe.end(), [] (studentas &j) {return j.galutinisVid>=5;});
//     vector<studentas>::iterator it1;
//     auto it = std::partition_point(grupe.begin(), grupe.end(), [] (studentas &j) {return j.galutinisVid>=5;});
//     for (it1=grupe.end(); it1!=it; it1--) {
//         pasidaveliai.push_back(grupe.back());
//         grupe.pop_back();
//     }
//     std::chrono::duration<double> dalinimas = std::chrono::high_resolution_clock::now() - start;
//     cout << "STUDENTU DALINIMAS I DVI GRUPES (OPTIMIZUOTAS): " << dalinimas.count() << " s" << endl;
// }
