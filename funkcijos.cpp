...
void rasytiIFaila(vector<studentas>& grupe, string pavadinimas) {
    ...
    if (!out.is_open()) {
        ...
        lentele(out);
        for (auto& i : grupe) {
            out << left << setw(15) << i.getName() << left << setw(15) << i.getSurname();
            out << std::fixed << std::setprecision(2) << left << setw(20) << i.getGalutinisVid() << left << setw(20) << i.getGalutinisMed() << endl;
        }
        out.close();
    }
}
    ...
    lentele(cout);
        for (auto &i:grupe){    
            ...
            cout << left << setw(15) << i.getName() <<
                left << setw(15) << i.getSurname() <<
                left << setw(20) << std::fixed << std::setprecision(2) << i.getGalutinisVid() << left << setw(20) << std::fixed << std::setprecision(2) << i.getGalutinisMed()<< endl;
        }   
